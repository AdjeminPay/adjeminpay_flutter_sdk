...
class OperatorPickerWidget extends StatefulWidget {
  ...
  final String? merchantTransactionData;
...
class _OperatorPickerWidgetState extends State<OperatorPickerWidget> {
  ...
  void loadData() {
    ...
    GatewayRepository()
       ...
       countryIso:widget.countryCode.toString().split('.').last)
        .then((value){
  ...
  void _createPayment(){
    ...
    GatewayRepository().createCheckout(
        clientId: widget.clientId,
        clientSecret: widget.clientSecret ,
        amount: widget.amount,
        currencyCode: widget.currencyCode,
        merchantTransId: widget.merchantTransactionId,
        merchantTransData: widget.merchantTransactionData,
  ...
  void _pay({required Customer customer, required GatewayOperator gatewayOperator, String? otp}){
    ...
    GatewayRepository().completeCheckout(
        clientId: widget.clientId,
        clientSecret: widget.clientSecret ,
      ...
      if(widget.isPayIn){
        ...
        if(gatewayOperator.name!.toLowerCase().contains('wave')){
          ...
          if(value.operatorTransUrl != null){
            //Open Payment URL
            //final Uri _paymentUrl = Uri.parse(value.paymentUrl!);
            _openPaymentUrl(value.operatorTransUrl!);
          }
